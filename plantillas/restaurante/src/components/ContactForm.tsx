...
import { NEGOCIO, HORARIOS } from "../consts";
...
export default function ContactForm() {
  ...
  return (
    <section id="contacto" className="bg-secondary py-20 text-text">
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-12 px-6 md:grid-cols-2">
        {/* Formulario */}
        <div>
          <h2 className="text-3xl font-bold text-primary sm:text-4xl">Contacto</h2>
          <p className="mt-3 text-text/70">Escribinos y te respondemos a la brevedad.</p>
          ...
          <form action={FORMSPREE_ACTION} method="POST" onSubmit={handleSubmit} className="mt-8 flex flex-col gap-4">
            <label className="flex flex-col gap-1 text-sm">
              ...
              <input
                ...
                name="nombre"
                required
                className="rounded-lg border border-text/20 bg-white px-4 py-2 text-text placeholder-text/40 outline-none focus:border-accent"
            ...
            {estado === "enviado" && (
              <p className="text-sm font-medium text-accent">¡Gracias! Tu mensaje fue enviado.</p>
            )}
            {estado === "error" && (
              <p className="text-sm font-medium text-red-600">Hubo un error. Probá de nuevo o escribinos por WhatsApp.</p>
            )}
          </form>
        </div>
